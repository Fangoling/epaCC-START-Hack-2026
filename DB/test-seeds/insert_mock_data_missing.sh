...
MOCK_DATA_FILE="DB/test-seeds/InsertMockData_Missing_MSSQL.sql"
...
docker cp "$MOCK_DATA_FILE" $CONTAINER_NAME:/InsertMockData_Missing_MSSQL.sql

echo "[INFO] Running mock data insertion script..."
# Run the SQL script inside the container using sqlcmd
docker exec -i $CONTAINER_NAME /opt/mssql-tools18/bin/sqlcmd -S localhost -U SA -P "$DB_PASS" -C -d $DB_NAME -i /InsertMockData_Missing_MSSQL.sql
